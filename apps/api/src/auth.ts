...
export function createSqlAuthService(pool: Pool, secret: string, now = () => new Date()): AuthService {
  ...
  const loadContext = async (client: Pool | PoolClient, userId: string, activeBranchId: string | null): Promise<AuthContext> => {
    const identity = await one<{ user_id: string; display_name: string; company_id: string; company_name: string; currency: string; timezone: string }>(client,
      `SELECT u.id user_id,u.display_name,u.company_id,c.name company_name,c.currency,c.timezone FROM users u JOIN companies c ON c.id=u.company_id WHERE u.id=$1 AND u.active AND c.active`, [userId]);
    ...
    const branches = (await client.query<{ id:string;name:string;code:string;settings:Record<string,unknown> }>(
      `SELECT b.id,b.name,b.code,COALESCE(bs.settings,'{}'::jsonb) settings FROM user_branch_access uba JOIN branches b ON b.id=uba.branch_id LEFT JOIN branch_settings bs ON bs.branch_id=b.id WHERE uba.user_id=$1 AND b.active ORDER BY b.name`, [userId])).rows;
    ...
    return { user: { id: identity.user_id, displayName: identity.display_name }, company: { id: identity.company_id, name: identity.company_name, currency: identity.currency, timezone: identity.timezone }, branches, activeBranch: active ?? null, permissions };
  };
  ...
  return {
    async login(input) { return withTransaction(pool, async (client) => {
      const user = await one<{ id:string }>(client, `SELECT u.id FROM users u JOIN companies c ON c.id=u.company_id WHERE u.company_id=$1 AND u.username=$2 AND u.active AND c.active AND u.password_hash = crypt($3,u.password_hash) FOR UPDATE`, [input.companyId, input.username, input.password]);
      if (!user) throw new AuthFailure();
      ...
      await client.query(`INSERT INTO auth_sessions(id,user_id,active_branch_id,refresh_token_hash,refresh_expires_at) VALUES($1,$2,(SELECT branch_id FROM user_branch_access WHERE user_id=$2 ORDER BY branch_id LIMIT 1),$3,$4)`, [sessionId,user.id,hash(refreshToken),expiry]);
      await client.query("UPDATE users SET last_login_at=NOW() WHERE id=$1", [user.id]);
      const session = await activeSession(client, sessionId, user.id); return materialize(client, session, refreshToken);
    }); },
